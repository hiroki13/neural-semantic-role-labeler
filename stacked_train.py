...
def get_id_samples(corpus, vocab_word, a_dict=None, sort=False):
    ...
    def get_args(prd_i, _sent):
        ...
        for w in _sent:
            ...
            if arg.startswith('('):
                ...
                if arg_label.endswith('V'):
                    _sent_args_coarse.append((arg_dict_c.get_id('B-V')))
                else:
                    _sent_args_coarse.append((arg_dict_c.get_id('B-A')))
            else:
                if prev:
                    arg_label = 'I-' + prev
                    if arg.endswith(')'):
                        prev = None
                    if arg_label.endswith('V'):
                        _sent_args_coarse.append((arg_dict_c.get_id('I-V')))
                    else:
                        _sent_args_coarse.append((arg_dict_c.get_id('I-A')))
                else:
                    arg_label = 'O'
    ...
    arg_dict_c.add_word('B-V')
    arg_dict_c.add_word('B-A')
    arg_dict_c.add_word('I-V')
    arg_dict_c.add_word('I-A')
...
def main(argv):
    ...
    n_ya = arg_dict_c.size()
    n_y = arg_dict.size()
    L2_reg = argv.reg
    batch = argv.batch

    """tagger setup"""
    tagger = LSTM(x=x, d=d, da=da, n_layers=argv.layer, op=argv.opt, n_in=n_in, n_h=n_hidden, n_ya=n_ya, n_y=n_y,
                  lr1=argv.lr1, lr2=argv.lr2, L2_reg=L2_reg)

    def _pre_train(_train_sample_x, _train_sample_y, _train_sample_ya):
        pre_train_model = theano.function(
            inputs=[x, d, da],
            outputs=[tagger.nll1, tagger.errors_a],
            updates=tagger.updates1,
            mode='FAST_RUN'
        )
        ...
        for epoch in xrange(5):
            ...
            for index in xrange(len(train_sample_x)):
                ...
                batch_ya = _train_sample_ya[index]
                ...
                for b_index in xrange(len(batch_x) / batch + 1):
                    ...
                    sample_ya = batch_ya[b_index * batch: (b_index + 1) * batch]
                    if len(sample_x) == 0:
                        continue

                    loss, error_a = pre_train_model(sample_x, sample_y, sample_ya)

                    losses.append(loss)
                    errors_a.extend(error_a)
            ...
            total = 0.0
            correct = 0
            for sent in errors_a:
                total += len(sent)
                for y_pred in sent:
                    if y_pred == 0:
                        correct += 1

            print '\tPre-Train BIO Accuracy: %f' % (correct / total)

    def _train(_train_sample_x, _train_sample_y, _train_sample_ya):
        train_model = theano.function(
            inputs=[x, d, da],
            outputs=[tagger.nll, tagger.errors, tagger.errors_a],
            updates=tagger.updates2,
            mode='FAST_RUN'
        )

        test_model = theano.function(
            inputs=[x, d, da],
            outputs=[tagger.y_pred, tagger.errors, tagger.errors_a],
            mode='FAST_RUN'
        )
...
#        _train_sample_x, _train_sample_y = shuffle(_train_sample_x, _train_sample_y)
        for epoch in xrange(argv.epoch):
            ...
            for index in xrange(len(train_sample_x)):
                ...
                for b_index in xrange(len(batch_x) / batch + 1):
                    ...
                    loss, error, error_a = train_model(sample_x, sample_y, sample_ya)
            ...
            print '\tTrain SRL Accuracy: %f' % (correct / total)
            ...
            print '\tTrain BIO Accuracy: %f' % (correct / total)
    ...
    def _test(model, epoch=0):
        ...
        for index in xrange(len(test_sample_x)):
            ...
            batch_y = test_sample_y[index]
            batch_yc = test_sample_yc[index]
            ...
            for b_index in xrange(len(batch_x)):
                ...
                sample_yc = batch_yc[b_index]

                pred, error, error_a = model([sample_x], [sample_y], [sample_yc])

                predicts.append(pred[0])
                errors.append(error[0])
                errors_a.append(error_a[0])
        ...
        for sent in errors:
            ...
            for y_pred in sent:
                if y_pred == 0:
                    correct += 1
        print '\tTest SRL Accuracy: %f' % (correct / total)
        ...
        for sent in errors_a:
            ...
            for y_pred in sent:
                if y_pred == 0:
                    correct += 1
        print '\tTest BIO Accuracy: %f' % (correct / total)

        f_measure(predicts, test_sample_y, test_arg_dict)
...
#    _pre_train(train_sample_x, train_sample_y, train_sample_yc)
    _train(train_sample_x, train_sample_y, train_sample_yc)
...
#                continue
    ...
    if p + r > 0:
        ...
        f = 0.
    print '\tBIO Props: %d\tP total: %f\tR total: %f\tCorrect: %f' % (len(predicts), p_total, r_total, correct)
    print '\tBIO Precision: %f\tRecall: %f\tF1: %f' % (p, r, f)
